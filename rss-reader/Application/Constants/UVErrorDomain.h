//
//  UVErrorDomain.h
//  rss-reader
//
//  Created by Uladzislau Volchyk on 2.01.21.
//

#ifndef UVErrorDomain_h
#define UVErrorDomain_h

static NSInteger const UVRSSReaderErrorCodeKey      = 10000;

static NSString *const UVNetworkErrorDomain         = @"com.uvolchyk.networkerror";
static NSString *const UVNullDataErrorDomain        = @"com.uvolchyk.nulldataerror";
static NSString *const UVPresentationErrorDomain    = @"com.uvolchyk.presentationerror";
static NSString *const UVDataWritingErrorDomain     = @"com.uvolchyk.datawrittingerror";

#endif /* UVErrorDomain_h */
